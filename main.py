...
def main(sites: tuple, limit: Optional[int], output: Optional[str], 
         output_format: Optional[str], list_sites: bool, 
         config_path: Optional[str], verbose: bool):
    """
    ...
    if not sites:
        ...
        if not sites:
            ...
            click.echo("--sites オプションでサイトを指定するか、--list-sites で利用可能なサイトを確認してください。", err=True)
            sys.exit(1)
    ...
    # 非同期実行
    asyncio.run(run_scraping(list(sites), limit or 30, output_path, output_format or 'json'))
